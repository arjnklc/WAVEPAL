...
class CarmaModel(object):
    ...
    def get_mle(self, p, q, ntrials=100, njobs=1):
        ...
        if njobs == 1:
            MLEs = list(map(_get_mle_single, args))
        else:
            # use multiple processors
            pool = multiprocessing.Pool(njobs)
            # warm up the pool
            pool.map(int, list(range(multiprocessing.cpu_count())))
...
class CarmaSample(samplers.MCMCSample):
    ...
    def __init__(self, time, y, ysig, sampler, q=0, filename=None, MLE=None):
        ...
        self.parameters = list(self._samples.keys())
        self.newaxis()
    ...
    def generate_from_trace(self, trace):
        ...
        if names != list(self._samples.keys()):
            idx = 0
